<?php

return [
    'brand' => 'GameList',
    'tagline' => 'Твоя игровая история — в одном месте',
    'hours' => '{1} :count час|[2,4] :count часа|[5,*] :count часов',
    'nav' => [
        'lists' => 'Мои списки', 'history' => 'Моя история', 'friends' => 'Мои друзья',
        'login' => 'Войти', 'register' => 'Регистрация', 'logout' => 'Выйти', 'settings' => 'Настройки',
    ],
    'actions' => [
        'create_list' => 'Новый список', 'save' => 'Сохранить', 'cancel' => 'Отмена', 'edit' => 'Изменить',
        'delete' => 'Удалить', 'add_game' => 'Добавить игру', 'catalog' => 'Поиск игр', 'import' => 'Импорт', 'preview' => 'Предпросмотр',
        'import_selected' => 'Импортировать выбранные', 'search' => 'Найти', 'copy' => 'Скопировать ссылку',
        'open' => 'Открыть список', 'back' => 'Назад к списку', 'cards' => 'Карточки', 'compact' => 'Компактно', 'board' => 'Доска',
    ],
    'statuses' => [
        'want_to_play' => 'Хочу сыграть', 'installed' => 'Установлена', 'playing' => 'Играю',
        'completed' => 'Пройдена', 'want_to_replay' => 'Хочу перепройти', 'replaying' => 'Перепрохожу',
        'completed_100' => 'Пройдена на 100%', 'dropped' => 'Брошена',
    ],
    'platforms' => [
        'nintendo_switch' => 'Nintendo Switch', 'steam' => 'Steam', 'pc' => 'PC',
        'playstation' => 'PlayStation', 'xbox' => 'Xbox',
    ],
    'profile_statuses' => [
        'want_to_play' => 'Хочу сыграть', 'installed' => 'Установлено', 'playing' => 'Играю',
        'completed' => 'Пройдено', 'want_to_replay' => 'Хочу перепройти', 'replaying' => 'Перепрохожу',
        'completed_100' => 'Пройдено на 100%', 'dropped' => 'Брошено',
    ],
    'activity' => [
        'online' => 'Онлайн',
        'unknown' => 'Последняя активность неизвестна',
        'last_seen' => 'Последняя активность: :time назад',
        'hours' => ':count час|:count часа|:count часов',
        'days' => ':count день|:count дня|:count дней',
        'months' => ':count месяц|:count месяца|:count месяцев',
    ],
    'counts' => [
        'friends' => '{1} друг|[2,4] друга|[5,*] друзей',
        'public_lists' => '{1} публичный список|[2,4] публичных списка|[5,*] публичных списков',
        'games' => '{1} игра|[2,4] игры|[5,*] игр',
        'additions' => '{1} добавление|[2,4] добавления|[5,*] добавлений',
        'ratings' => '{1} оценка|[2,4] оценки|[5,*] оценок',
    ],
    'messages' => [
        'registered' => 'Добро пожаловать в GameList!', 'list_created' => 'Список создан.',
        'list_updated' => 'Список обновлён.', 'list_deleted' => 'Список удалён.',
        'game_created' => 'Игра добавлена.', 'game_updated' => 'Игра обновлена.',
        'game_deleted' => 'Игра удалена.', 'status_updated' => 'Статус обновлён.',
        'imported' => 'Добавлено игр: :count.', 'copied' => 'Ссылка скопирована.',
        'avatar_updated' => 'Аватар обновлён.', 'email_updated' => 'Email изменён.', 'password_updated' => 'Пароль изменён.',
        'steam_connected' => 'Аккаунт Steam подключён.', 'steam_disconnected' => 'Аккаунт Steam отключён.',
        'steam_login_complete' => 'Вы вошли через Steam.',
        'steam_library_imported' => 'Список создан. Добавлено игр из Steam: :count.',
        'steam_library_exists' => 'Список «Игры из Steam» уже создан.',
        'profile_cover_updated' => 'Обложка профиля обновлена.', 'favorites_updated' => 'Любимые игры обновлены.',
        'friend_added' => 'Пользователь добавлен в друзья.', 'friend_removed' => 'Пользователь удалён из друзей.',
        'notification_removed' => 'Уведомление удалено.', 'notifications_cleared' => 'Уведомления очищены.',
        'password_reset_code_sent' => 'Если аккаунт с таким email существует, код уже отправлен.',
        'password_reset_complete' => 'Пароль изменён. Теперь войдите с новым паролем.',
        'game_added_to_list' => 'Игра добавлена в список «:list».',
        'review_saved' => 'Оценка и мнение сохранены.', 'review_deleted' => 'Оценка и мнение удалены.',
    ],
    'errors' => [
        'cover_download' => 'Не удалось скачать обложку. Попробуйте другой URL или загрузите файл.',
        'cover_not_image' => 'По указанному URL находится не изображение.',
        'cover_size' => 'Обложка должна быть меньше 8 МБ.',
        'cover_invalid' => 'Файл не удалось распознать как изображение.',
        'cover_url' => 'Этот URL нельзя использовать для загрузки обложки.',
        'steam_auth_failed' => 'Не удалось подтвердить вход через Steam. Попробуйте ещё раз.',
        'steam_auth_expired' => 'Сессия входа через Steam устарела. Начните вход заново.',
        'steam_already_connected' => 'Этот аккаунт Steam уже привязан к другому пользователю.',
        'steam_password_required' => 'Сначала создайте пароль, чтобы не потерять доступ после отключения Steam.',
        'steam_connection_required' => 'Сначала привяжите аккаунт Steam в настройках.',
        'steam_api_key_missing' => 'Импорт Steam пока не настроен: отсутствует STEAM_WEB_API_KEY.',
        'steam_api_key_invalid' => 'Steam отклонил ключ Web API. Проверьте STEAM_WEB_API_KEY.',
        'steam_library_private' => 'Steam не разрешил прочитать библиотеку. Откройте доступ к разделу «Детали игр» в настройках приватности.',
        'steam_library_unavailable' => 'Steam сейчас не отвечает. Попробуйте импортировать библиотеку позже.',
        'steam_library_invalid' => 'Steam вернул некорректные данные библиотеки. Попробуйте ещё раз позже.',
        'steam_import_failed' => 'Не удалось создать список из библиотеки Steam.',
        'game_duplicate' => 'Такая игра уже есть в этом списке.',
        'statuses_in_use' => 'Нельзя отключить статусы, которые уже используются: :statuses. Сначала измените статусы этих игр.',
        'import_limit' => 'За один раз можно импортировать не более 100 игр.',
        'favorite_games_owner' => 'Можно выбрать только игры из собственных списков.',
        'login_reserved' => 'Этот логин зарезервирован системой. Выберите другой.',
        'password_reset_code_invalid' => 'Код неверный, просрочен или больше не действует.',
        'password_reset_throttled' => 'Слишком много запросов. Повторите через :seconds сек.',
        'review_required' => 'Поставьте оценку или напишите мнение об игре.',
        'status_not_available' => 'Этот статус недоступен в выбранном списке.',
    ],
];
