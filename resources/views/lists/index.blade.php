...
@else
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @foreach ($lists as $list)
            <a href="{{ route('lists.show', $list) }}" class="glass group rounded-3xl p-5 transition duration-300 hover:-translate-y-1 hover:border-violet-400/30 hover:bg-white/[.075]">
                ...
                <h2 class="mt-6 text-xl font-extrabold tracking-tight transition group-hover:text-violet-200">{{ $list->name }}</h2>
                <p class="mt-2 line-clamp-2 min-h-12 text-sm leading-6 text-slate-500">{{ $list->description ?: 'Игровая коллекция без описания.' }}</p>
                <div class="mt-5 flex items-center justify-between border-t border-white/7 pt-4 text-xs font-semibold text-slate-500">
