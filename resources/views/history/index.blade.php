...
<div class="mx-auto max-w-4xl">
    <div class="mb-8">
        ...
        <p class="muted mt-3 max-w-2xl">Все даты начала и завершения игр из ваших списков. Самые свежие события находятся наверху.</p>
    </div>
    ...
    @if ($eventsByDate->isEmpty())
        <div class="panel flex min-h-72 flex-col items-center justify-center text-center">
            ...
            <p class="muted mt-2 max-w-lg">Укажите дату начала или завершения у игры — событие автоматически появится здесь.</p>
            ...
            @foreach ($eventsByDate as $date => $events)
                <section class="relative pb-9 pl-12 last:pb-0">
                    ...
                    <time datetime="{{ $date }}" class="block text-xs font-extrabold uppercase tracking-[.12em] text-violet-300">
                        {{ $events->first()['date']->translatedFormat('j F Y') }}
                    </time>

                    <div class="mt-3 space-y-3">
                        @foreach ($events as $event)
                            @php($game = $event['game'])
                            @php($completed = $event['type'] === 'completed')
                            @php($duration = $completed ? $game->completionDuration() : null)
                            <a href="{{ route('games.edit', $game) }}" class="glass group flex gap-3 overflow-hidden rounded-2xl p-3 transition hover:-translate-y-0.5 hover:border-white/20 sm:gap-4 sm:p-4" data-history-event>
                                ...
                                <div class="min-w-0 flex-1 py-0.5">
                                    <p class="flex flex-wrap items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-[.11em] {{ $completed ? 'text-emerald-300' : 'text-cyan-300' }}">
                                        <span class="material-symbols-outlined text-sm">{{ $completed ? 'trophy' : 'play_circle' }}</span>
                                        <span>{{ $completed ? 'Прошёл игру' : 'Начал играть' }}</span>
                                        @if ($duration)
