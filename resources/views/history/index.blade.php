...
@section('title', $isOwner ? __('app.nav.history') : 'История @'.$profile->login)

@section('content')
<div class="mx-auto max-w-4xl">
    <div class="mb-8">
        <span class="eyebrow"><span class="material-symbols-outlined">history</span> Игровая хронология</span>
        <h1 class="page-title">{{ $isOwner ? __('app.nav.history') : 'История @'.$profile->login }}</h1>
        @if ($isOwner)
            <p class="muted mt-3 max-w-2xl">Все даты завершения игр из ваших списков. Самые свежие события находятся наверху.</p>
        @else
            <p class="muted mt-3 max-w-2xl">Все даты завершения игр из публичных списков {{ '@'.$profile->login }}. Самые свежие события находятся наверху.</p>
        @endif
    </div>
    ...
    @if ($gamesByCompletionDate->isEmpty())
        <div class="panel flex min-h-72 flex-col items-center justify-center text-center">
            ...
            @else
                <p class="muted mt-2 max-w-lg">В публичных списках {{ '@'.$profile->login }} пока нет завершённых игр.</p>
            @endif
        </div>
    @else
        <div class="relative">
